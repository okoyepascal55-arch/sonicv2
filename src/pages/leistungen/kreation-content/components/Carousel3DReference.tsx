...
const gradient = (hue: number) =>
  `linear-gradient(135deg, oklch(0.24 0.05 ${hue}), oklch(0.13 0.005 118))`;
...
export default function Carousel3DReference() {
  ...
  const tiles = useMemo(() => LABELS.map((label, i) => ({
    alt: label,
    src: images[i]?.url ? resolveImageUrl(images[i].url) : FALLBACKS[i],
  })), [images]);
  ...
  return (
    <div
      ...
      style={{ height: '480px', perspective: '1800px', margin: '0 -40px' }}
          ...
          >
            ...
            >
              <img
                ...
                draggable={false}
              />
              <div className="absolute bottom-0 left-0 right-0 h-[30px] bg-white flex items-center justify-center">
                <span className="text-[9px] font-black tracking-[0.15em] uppercase text-black/40">
                  {tile.alt}
                </span>
              </div>
            </div>
